...
@[simp]
def STree.abs {α : Type} (t : STree α) : Assoc α := t.elements
...
theorem pull_cons_from_if {α : Type} {b1 : Prop} {b2 : Prop} [Decidable b1] [Decidable b2] (a : α) (l1 l2 l3 : List α)
  : a :: (if b1 then l1 else if b2 then l2 else l3) = if b1 then a :: l1 else if b2 then a :: l2 else a :: l3 := by
  by_cases b1 <;> simp [*]
  by_cases b2 <;> simp [*]

theorem bound_assoc_not (l : Assoc α) (k : Nat)
...
theorem bound_assoc_split {α : Type} (k k' : Nat) (v' : α) (l1 l2 : Assoc α)
...
theorem lookup_assoc_not (l : Assoc α) (k : Nat)
...
theorem lookup_assoc_split (k k' : Nat) (v' : α) (l1 l2 : Assoc α)
  : Forall (fun (k0, _) => k0 < k') l1 ->
    Forall (fun (k0, _)     => k0 > k') l2 ->
...
theorem insert_assoc_split {α : Type} (k k' : Nat) (v v' : α) (l1 l2 : Assoc α)
  : Forall (fun (k0, _) => k0 < k') l1 ->
    Forall (fun (k0, _)     => k0 > k') l2 ->
